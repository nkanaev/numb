...
func Asin(args ...value.Value) value.Value {
	arity(1, len(args))
	return asRadian(value.Float64(math.Asin(f64(args[0]))))
}

func Acos(args ...value.Value) value.Value {
	arity(1, len(args))
	return asRadian(value.Float64(math.Acos(f64(args[0]))))
}

func Atan(args ...value.Value) value.Value {
	arity(1, len(args))
	return asRadian(value.Float64(math.Atan(f64(args[0]))))
}
